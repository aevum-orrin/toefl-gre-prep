# vocab-srs — spaced-repetition vocabulary trainer

A shared TOEFL/GRE flashcard web page driven by `prep_core.SRS` (the SM-2 algorithm, the same family
Anki started with). Scheduling only — no network cost; pronunciation uses the browser's built-in
speech synthesis (free, zero storage).

```bash
source ../../env.sh
uvicorn app:app --reload --port 8003   # http://localhost:8003
```

## How to use
- Switch **TOEFL / GRE / scenes** decks at the top (wordlists come from `toefl/vocab/` and `gre/vocab/`).
- See the word → **Enter** = "I know it cold", never shown again; or **Space** to reveal → grade:
  - **Again (1)** didn't recall → comes back later **today**, but only after ≥15 other cards
    (spacing-effect guard — a lapsed word never pops right back)
  - **Hard (2)** / **Good (3)** → recalled; the interval grows per SM-2
- Keyboard: `Space` reveal · `1/2/3` grade · `4` 🔊 pronounce (US male, works on **any** word
  screen — front or revealed, ordinary card or one opened from the lookup) · `Enter` know-it ·
  `←` undo · `→` redo · `/` lookup.

## Scheduling algorithm (SM-2 + proficiency)
Only a word confirmed on FIRST sight may disappear immediately; everything else must earn it:

| Action | Effect |
|---|---|
| **Enter, first sight** | graduated — never shown again |
| **Enter, seen before (never Again'd)** | strong pass; the word returns **once more for confirmation** — a second consecutive Enter graduates it, any 1/2/3 in between resets the streak |
| **Enter, ever graded Again** | refused forever (sticky-Again 🔁); counted as a Good review |
| **1 Again** | due again today, but only after ≥15 other cards; lapses **preempt** new words once the gap has passed |
| **2 Hard / 3 Good** | SM-2 day intervals (1 → 6 → ×ease), scaled by proficiency |

**Proficiency (`prof`, shown as 熟练度)** is a reward-EMA over the card's whole grade history
(again/hard→0, good→0.8, enter→1.0; α=0.4) — the "reinforcement" signal: recent evidence
dominates, but an early lapse keeps a word on shorter intervals than an always-easy one
(interval ×(0.5+prof)). Failing also drops ease by 0.2 (Anki-style).

**Card order** (user's rule: never-seen words strictly before seen ones):
today's Again-lapses past the 15-card gap → **new words** (tier/TPO order) → other due reviews.
With new/day = ∞ the whole deck is new, so day-level reviews only surface after you pause new
introduction — pick a finite new/day (e.g. 300) if you want yesterday's reviews interleaved daily.

## Pronunciation & phonetics
- Every card shows **US and UK IPA** (merged from open-dict-data/ipa-dict by `scripts/add_ipa.py`)
  with **4 speaker buttons**: US ♂ / US ♀ / UK ♂ / UK ♀.
- Two engines (pick in 🔊⚙; default = server):
  - **Server neural voices** (`/api/tts`, edge-tts → Andrew/Aria/Ryan/Sonia): free Microsoft
    endpoint, needs outbound internet on the serving node; mp3s cached forever on scratch
    (`$LANG_PREP_CACHE/tts/`, ~9 KB/word — ~150 MB realistic, ≈0.5 GB absolute worst case).
    Works in any browser, immune to browser TTS bugs.
  - **Browser Web Speech API** (fallback): zero storage, but macOS Chrome randomly drops
    utterances ("canceled", a known Chromium bug) and VNC desktops have no voices at all.
    Voice per slot is heuristic; override in 🔊⚙ (saved in localStorage).
- Each new card (and each undo) **auto-plays** once with the US male voice; toggle in 🔊⚙.
- Each part-of-speech row also has an IPA + a US-male speaker button.

## On the reveal page
- **🌱 词根词缀 · 词源** — collapsed block above the primary definition (roots/affixes glossed in
  Chinese, meaning derivation, source language / loanword note). Generated by
  `scripts/enrich_etym.py` (free LLM, resumable cache on scratch); words without a genuinely
  helpful analysis show no block.
## 🔎 词汇检索 — a global floating widget
Like the timer, the lookup box floats over **every** screen: a word's front side (before Space),
a revealed answer, even the "nothing due" screen. Open it with the bottom-left button, `/`, or
`Ctrl`/`⌘`+`K`; `↑`/`↓` pick a hit, `Enter` opens it, `Esc` closes.

- Contiguous-substring match within the **current deck only** ("sub" → subsequent, submarine…).
- Clicking a hit opens that word as a **full study page** — grading it goes through the normal
  `/api/review`, so it counts toward today.
- **⬅ 返回检索前** steps back **one word per press**, however deep the chain (A → B → C → three
  presses land back on A, restored exactly as it was, revealed or not). The button shows the
  remaining depth and lives both in the panel and in the action row. This is a true back-stack —
  distinct from `←` undo, which reverts *grades*.
- **📝 Personal Notes** — collapsed markdown notes box (Chinese/English/anything; paste GPT answers
  as-is). Autosaves to `$PREP_DATA_DIR/notes/vocab_notes.json`, keyed by term and shared across
  decks, so the note is there whenever you meet the word again. Rendered with vendored
  marked.js + DOMPurify (`static/vendor/`).

## Data
- Wordlist (content): deck JSON under each exam's `vocab/` folder (`term, senses, ipa_us, ipa_uk,
  etymology, tier, …`); built by `scripts/build_vocab.py`, enriched by `scripts/enrich_vocab.py` /
  `enrich_etym.py` / `add_ipa.py`.
- Review state (scheduling): `$PREP_DATA_DIR/srs/<deck>.json`, saved automatically on each grade.
- Personal notes: `$PREP_DATA_DIR/notes/vocab_notes.json`.
- Adding words: append to the wordlist JSON; new terms are merged in on startup and existing review
  progress is untouched.

> SM-2 is plenty for a personal sprint. To minimize review load later you can swap the scheduler for
> FSRS (Anki's current default) without changing the rest of the app — see the research notes under
> `docs/`.
